...
fn validate(input: &str, data_validate: bool) -> usize {
    ...
    let re_hgt = Regex::new(r"^(?P<cm>\d+)cm|(?P<in>\d+)in$").unwrap();
    let mut valid = 0;
    ...
    for passport in input.split("\n\n") {
        ...
        for field in passport.split(char::is_whitespace) {
            if !field.is_empty() {
                ...
                if data_validate {
                    match key {
                        "byr" => v.byr = (1920..=2002).contains(&val.parse().unwrap()),
                        "iyr" => v.iyr = (2010..=2020).contains(&val.parse().unwrap()),
                        "eyr" => v.eyr = (2020..=2030).contains(&val.parse().unwrap()),
                        "hgt" => {
                            v.hgt = {
                                re_hgt.captures(val).map_or(false, |cap| {
                                    cap.name("cm").map_or(false, |n| {
                                        (150..=193).contains(&n.as_str().parse().unwrap())
                                    }) || cap.name("in").map_or(false, |n| {
                                        (59..=76).contains(&n.as_str().parse().unwrap())
                                    })
                                })
                            }
                        }
                        "hcl" => v.hcl = re_hcl.is_match(val),
                        "ecl" => v.ecl = re_ecl.is_match(val),
                        "pid" => v.pid = re_pid.is_match(val),
                        "cid" => {}
                        _ => unreachable!(),
                    }
                } else {
        ...
        if v.byr && v.iyr && v.eyr && v.hgt && v.hcl && v.ecl && v.pid {
            valid += 1;
        }
    }
...
"#;
        ...
        assert_eq!(pt2(&invalid), 0);
        assert_eq!(pt2(&valid), 4);
    }
}

pub static INPUT: &str = r#"eyr:2029 iyr:2013
