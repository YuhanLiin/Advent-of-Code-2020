pub fn pt1(input: &str) -> usize {
    ...
    for group in input.split("\n\n") {
        ...
        for ch in group.bytes() {
            if (b'a'..=b'z').contains(&ch) && !found[(ch - b'a') as usize] {
                found[(ch - b'a') as usize] = true;
                total += 1;
            }
        }
    }
...
pub fn pt2(input: &str) -> usize {
    ...
    for group in input.split("\n\n") {
        ...
        for line in group.lines() {
            if !line.is_empty() {
                let answered = line.bytes().fold(0u32, |ans, ch| ans | (1 << (ch - b'a')));
                found &= answered;
            }
        }
        total += found.count_ones() as usize;
    }
...
pub static INPUT: &str = r#"gsvdkufnoawjmhp
wvhusojpnikgfadb
vshnpfedgwajkou
aujodhskfvnpgw
okpdnwhsfvjguqa
